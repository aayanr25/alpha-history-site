...
async function onRequest2(context) {
  ...
  try {
    do {
      ...
      if (!res.ok) {
        return json2({ error: `Drive API error ${res.status}`, detail: await res.text() }, 502);
      }
    ...
    return json2({ error: "Could not reach Google Drive", detail: String(err) }, 502);
  }
  const photos = [];
  for (const file of files) {
    const match2 = file.name.match(/^(\d+)/);
    if (!match2) continue;
    photos.push({
      initiation_number: Number(match2[1]),
      url: `/api/image?id=${file.id}&w=400`
    });
  }
  return json2(photos, 200, { "cache-control": "public, max-age=300" });
}
...
async function onRequest3(context) {
  ...
  try {
    ...
    if (!res.ok) {
      return json3({ error: `Sheets API error ${res.status}`, detail: await res.text() }, 502);
    }
    const data = await res.json();
    rows = data.values || [];
  } catch (err) {
    return json3({ error: "Could not reach Google Sheets", detail: String(err) }, 502);
  }
  if (rows.length < 2) {
    return json3([], 200, { "cache-control": "public, max-age=300" });
  }
  const header = rows[0].map((h) => String(h || "").trim().toLowerCase());
  const col = /* @__PURE__ */ __name((name) => header.indexOf(name), "col");
  const idx = {
    order: col("order"),
  ...
  const cell = /* @__PURE__ */ __name((row, i) => i >= 0 && row[i] != null ? String(row[i]).trim() : "", "cell");
  const entries = [];
  ...
  return json3(entries, 200, { "cache-control": "public, max-age=300" });
}
...
// ../.wrangler/tmp/pages-VF986J/functionsRoutes-0.8594270313562586.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-0nPODY/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-0nPODY/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.01935164671865164.mjs.map
